...
import sys
sys.path.insert(0, "/private/task/huangmq/CogVideo/finetune/models")
from transformer_ipadapter import GeneralIP_CogVideoXTransformer3DModel
from pipeline_ipadapter import GeneralIP_Pipeline
...
def generate_video(
    ...
    if ipadapter_path:
        checkpoint = torch.load(os.path.join(ipadapter_path, "ipadapter.pth"))
        # 加载 state_dict 到对应模型中
        transformer.add_blocks_for_ref(is_training=False)
        blocks_for_ref_load_result = transformer.blocks_for_ref.load_state_dict(checkpoint["blocks_for_ref"])
        map_load_result =transformer.map.load_state_dict(checkpoint["map"])
        print(f"blocks_for_ref_load_result:{blocks_for_ref_load_result}")
        print(f"map_load_result:{map_load_result}")
        transformer.blocks_for_ref = transformer.blocks_for_ref.to(dtype=dtype)
        transformer.map = transformer.map.to(dtype=dtype)
    ...
    clip_model, clip_preprocess = clip.load("/private/task/huangmq/CogVideo/finetune/models/clip/ViT-L-14.pt",device=device)
    ...
    if generate_type == "i2v":
        ...
        image = Image.open(image_or_video_path).convert("RGB")
        image = clip_preprocess(image).unsqueeze(0).to(device)
        with torch.no_grad():
            image_features = clip_model.encode_image(image).unsqueeze(0).to(dtype=dtype)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str, default="finetune/CogVideoX-2b", help="The path of the pre-trained model to be used"
    )
    parser.add_argument(
        "--ipadapter_path", type=str, help="The path of pre_Transformer"
    )
    ...
    generate_video(
        ...
        ipadapter_path = args.ipadapter_path,
