...
from diffusers import AutoencoderKLCogVideoX, CogVideoXDPMScheduler, CogVideoXPipeline, CogVideoXTransformer3DModel,CogVideoXDDIMScheduler
from models.transformer_ipadapter import GeneralIP_CogVideoXTransformer3DModel
...
def main(args):
    ...
    transformer.add_blocks_for_ref(is_training=True)
    ...
    for param in transformer.blocks_for_ref.parameters():
        param.requires_grad = True
    for param in transformer.map.parameters():
        param.requires_grad = True
    ...
    clip_model, clip_preprocess = clip.load("models/clip/ViT-L-14.pt", device=accelerator.device)
        ...
        if accelerator.is_main_process:
            for model in models:
                if isinstance(model, type(unwrap_model(transformer))):
                    checkpoint = {
                                    "blocks_for_ref": model.blocks_for_ref.state_dict(),
                                    "map": model.map.state_dict(),
                                }
                    torch.save(checkpoint, os.path.join(output_dir,"ipadapter.pth"))
                else:
    ...
    def load_model_hook(models, input_dir):
        ...
        lora_state_dict = GeneralIP_Pipeline.lora_state_dict(input_dir)
    ...
    with open("train_params_ipadapter.txt","w") as file:
    ...
    def process_image(image_paths):
        ...
        for image_path in image_paths:
            ...
            image = clip_preprocess(image).unsqueeze(0).to(accelerator.device)
            with torch.no_grad():
                image_features = clip_model.encode_image(image)
            images_features.append(image_features.unsqueeze(0))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                ref_images = batch["instance_ref_image"] #[b,3,480,720]
                videos = videos.permute(0, 2, 1, 3, 4)
                model_input = encode_video(videos).to(dtype=weight_dtype) #[2,13,16,60,90]
                ref_images = process_image(ref_images).to(dtype=weight_dtype) #[b,1,768]
    ...
    if accelerator.is_main_process:
        ...
        # )
        torch.save(transformer.blocks_for_ref.state_dict(), os.path.join(args.output_dir,"attn1_for_ref_image.pth"))
